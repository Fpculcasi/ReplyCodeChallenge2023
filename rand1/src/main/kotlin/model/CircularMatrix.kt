...
class CircularMatrix(var matrix: List<List<String>> = ArrayList(ArrayList()), val c: Int, val r: Int) {
    ...
    fun pointsAt(coordinate: Coordinate) =
        matrix[coordinate.y][coordinate.x].let {
            try {
                it.toInt()
            } catch (e: Exception) {
                0
    ...
    fun isWormHole(coordinate: Coordinate) = matrix[coordinate.y][coordinate.x] == "*"
    ...
    fun nextMove(position: Coordinate): Direction {
        return if (position.x == c - 1) {
            if(position.y%2 == 0) {
                Direction.D
            } else {
                Direction.L
